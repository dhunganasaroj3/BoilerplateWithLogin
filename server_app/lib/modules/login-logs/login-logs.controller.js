const loggedInController = (() => {
  ...
  const moduleConfig = require('./login-logs.config');
  ...
  _p.getFailedLoginAttemptCount = (req, res, user_id) => {
    ...
    return  req.db.collection('LoginLogs').estimatedDocumentCount(queryOpts);
  };
  ...
  _p.updateLoggedInInfo = (req, user_id) => {
    ...
    return req.db.collection('LoginLogs').updateMany(queryOpts, updateOpts);
  };
