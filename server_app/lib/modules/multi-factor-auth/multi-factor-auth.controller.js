const multiFactorAuthController = (() => {
  ...
  _p.verifyTwoFactorAuthentication = async (req, updateOpts, user_id) => {
    return req.db.collection('User').updateOne({ _id : user_id }, {
      $set: updateOpts
    });
  };
  ...
  _p.disableMultiFactorAuthHelperFunc = async (req, res, updateOpts, next) => {
    const _userId = (utilityHelper.containsElementInArr(commonHelper.getLoggedInUserRole(req), roleConfig.superadmin, next)) ? req.params.userId : commonHelper.getLoggedInUserId(req);
  ...
  _p.validateTOTPToken = async (req, res, next) => {
    try {
      if (req.body.totp_token) {
        ...
        const userInfo = await req.db.collection('User').findOne({ _id: req.params.userId, deleted: false });
        if (userInfo) {
            ...
            if(userInfo && userInfo.backup_recovery_codes && userInfo.backup_recovery_codes.includes(req.body.totp_token)) {
              ...
              if(dataRes.result.n > 0) {
                req.db.collection('User').updateOne(
                    { _id: req.params.userId, deleted: false }, {
                ...
                if(req.loginStatusMessage.userInfo && req.loginStatusMessage.userInfo.user_role) {
                  let found = [roleConfig.superadmin, roleConfig.enduser, roleConfig.vendor, roleConfig.independent_marketing_partner].some((role) => req.loginStatusMessage.userInfo.user_role.indexOf(role) >= 0)
                  if(!found) {
                    const iamActions = await identityAccessManagementRoleController.getAttachedActionsWithGroupByRole(req, req.loginStatusMessage.userInfo.user_role[0], req.loginStatusMessage.userInfo.parent_id, next);
                    if(iamActions && iamActions.length > 0) {
                      req.loginStatusMessage.allowed_actions = jwtTokenGeneratorHelper.generateAllowedActions(req, iamActions, req.loginStatusMessage.userInfo._id);
                    }
                  }
                }
                return commonHelper.sendJsonResponse(res, req.loginStatusMessage, '', HTTPStatus.OK);
  ...
  _p.validateMobileTOTPToken = async (req, res, next) => {
    try {
      if (req.body.totp_token) {
        const userObj = await req.db.collection('User').findOne({ _id: req.params.userId, deleted: false });
  ...
  _p.verifyTOTPSecret = async (req, res, next) => {
    try {
      ...
      if (verified) {
        ...
        const _userId = commonHelper.getLoggedInUserId(req);
  ...
  _p.verifyMobileTOTPSecret = async (req, res, next) => {
    try {
      const twoFactorAuthSecret = (req.query && req.query.secret) ? req.query.secret : '';
      const userObj = await req.db.collection('User').findOne({_id: commonHelper.getLoggedInUserId(req), deleted: false});
      if(userObj.multi_factor_auth_secret_mobile === twoFactorAuthSecret) {
        const userRes = await req.db.collection('User').updateOne( { _id: commonHelper.getLoggedInUserId(req) }, {$set: {
  ...
  _p.sendMultiFactorMobileCode = async (req, user_id, userObj) => {
    ...
    const dataRes = await req.db.collection('User').updateOne( { _id: user_id }, {$set: {
  ...
  _p.generateMobileMultiFactorAuthCode = async (req, res, next) => {
    try {
      ...
      if (userObj && userObj.country_code==="" && userObj.mobile_number==="") {
        let message = '';
        if(userObj.mobile_number===""){
          message = moduleConfig.message.validationErrMessage.mobile_number;
        } else {
          message = moduleConfig.message.validationErrMessage.country_code;
        }
        return commonHelper.sendResponseData(res, {
  ...
  _p.generateRecoveryCode = async (req, res, next) => {
    try {
      ...
      const recoveryRes = await  req.db.collection('User').updateOne({ _id : commonHelper.getLoggedInUserId(req) }, updateOpts);
    ...
    try {
      ...
      if (req.params.templateId) {
        ...
        if (emailTemplateInfo && emailTemplateInfo.template_content) {
          ...
          if (messageBody.indexOf("%person_name%") > -1) {
            messageBody = messageBody.replace("%person_name%", `${userInfo.first_name} ${userInfo.last_name}`);
          }
          const recoveryCodes = userInfo.backup_recovery_codes.map((item, index) => `<span>${item}</span>`);
          if (messageBody.indexOf("%recovery_code%") > -1) {
            messageBody = messageBody.replace("%recovery_code%", recoveryCodes.join(","));
          }
